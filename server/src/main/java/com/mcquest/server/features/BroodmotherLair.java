...
import com.mcquest.server.npc.BroodmotherLairAdventurer;
...
public class BroodmotherLair implements Feature {
    ...
    private void guardThomas() {
        ...
        GuardThomas guardThomas = new GuardThomas(mmorpg, Instances.BROODMOTHER_LAIR, position);
        mmorpg.getObjectManager().add(guardThomas);
    }
    ...
    private void adventurers() {
        ...
        for (Pos position : positions) {
            BroodmotherLairAdventurer adventurer =
                    new BroodmotherLairAdventurer(mmorpg, Instances.BROODMOTHER_LAIR, position);
            objectManager.add(adventurer);
        }
    }
    ...
    private void spiders() {
        ...
        ObjectManager objectManager = mmorpg.getObjectManager();
        for (Pos position : positions) {
            Spider spider = new Spider(mmorpg, Instances.BROODMOTHER_LAIR, position);
            objectManager.add(spider);
        }
    }
    ...
    private void lootChests() {
        ...
        LootChest lootChest1 = new LootChest(
                Instances.BROODMOTHER_LAIR,
                new Pos(63.5, 30.0, 214.5, 125.0f, 0.0f),
                LootTable.builder()
                        .pool(Pool.builder()
        ...
        lootChest1.setRespawnDuration(Duration.ofSeconds(120));
        objectManager.add(lootChest1);
    }
}
