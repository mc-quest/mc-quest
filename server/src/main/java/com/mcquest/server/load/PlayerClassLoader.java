...
import com.mcquest.server.playerclass.PlayerClassBuilder;
...
public class PlayerClassLoader {
    public static void loadPlayerClasses(Mmorpg mmorpg) {
        PlayerClassManager playerClassManager = mmorpg.getPlayerClassManager();
        List<String> paths = ResourceUtility.getResources("playerclasses");
        for (String path : paths) {
            JsonObject object = ResourceUtility.getResourceAsJson(path).getAsJsonObject();
            int id = object.get("id").getAsInt();
            String name = object.get("name").getAsString();
            PlayerClassBuilder builder = playerClassManager.playerClassBuilder(id, name);
